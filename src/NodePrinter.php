...
//    }
	...
	public function pExpr_Array(Node\Expr\Array_ $node) {
		...
		$arr = '[' . $this->pCommaSeparated($node->items) . ']';
		if (strlen($arr) > 72) {
...
//	{
//		//return parent::pCommaSeparated($nodes); // TODO: Change the autogenerated stub
//		return $this->pCommaSeparatedLines($nodes);
//	}
	...
	public function pExpr_New(Expr\New_ $node) {
		...
		return 'new ' . $this->p($node->class) . '(' . $this->pCommaSeparatedLines($node->args) . ')';
	}

	protected function pCommaSeparatedLines(array $nodes)
	{
//		$arr = $this->pCommaSeparated($nodes);
